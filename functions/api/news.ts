...
import type { NewsArticle, NewsFetchResponse } from "../../src/lib/news-sources";
...
function extractTag(xml: string, tagName: string): string {
  ...
  const standardRe = new RegExp(`<${tagName}[^>]*>([\\s\\S]*?)<\\/${tagName}>`, "i");
...
function parseRssFeed(xml: string, sourceId: string, sourceName: string): NewsArticle[] {
  ...
  for (let i = 0; i < itemMatches.length; i++) {
    ...
    const descriptionRaw = extractTag(itemXml, "description") || extractTag(itemXml, "summary") || extractTag(itemXml, "content");

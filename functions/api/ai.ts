...
export const aiRouter = new Hono<{ Bindings: Bindings }>()
  .post('/global-chat', zValidator('json', z.object({
    ...
    const settings = await db
      .prepare("SELECT gemini_api_key, gemini_model, global_chat_history FROM user_settings WHERE user_id = ?")
      .bind(userId)
      .first<{ gemini_api_key: string | null; gemini_model: string | null; global_chat_history: string | null }>();

    if (!settings || !settings.gemini_api_key) {
      return c.json({ error: "Gemini API Key not configured in settings." }, 400);
    }
    ...
    const libraryRows = await db.prepare(`
...
You have access to Google Search grounding tools; use them actively to search the web and provide current, up-to-date information for recent events, releases, chapter/episode updates, news, or general real-time queries. Keep responses concise and use markdown formatting.`;
    ...
    try {
        ...
        const response = await chatSession.sendMessage({ message: data.message });
        ...
        history.push({ role: 'model', text: aiText, thought: aiThought || undefined });

        await db.prepare("UPDATE user_settings SET global_chat_history = ? WHERE user_id = ?")
            .bind(JSON.stringify(history), userId)
            .run();

        return c.json({ text: aiText, thought: aiThought, history });
    } catch (e: any) {
        return c.json({ error: e.message || "Failed to communicate with AI." }, 500);
    }
  })
  .post('/card-chat/:id', zValidator('json', z.object({
    message: z.string().min(1),
  })), async (c) => {
    ...
    const encryptionKey = c.env.ENCRYPTION_KEY ?? "fallback-encryption-key-for-local-dev-123";
    let apiKey = await decryptApiKey(settings.gemini_api_key, encryptionKey);
...
You have access to Google Search grounding tools; use them actively to search the web and provide current, up-to-date information for recent events, releases, chapter/episode updates, news, or general real-time queries. Keep responses concise and use markdown formatting.`;
    ...
    try {
        ...
        await db.prepare("UPDATE user_media SET ai_chat_history = ? WHERE id = ?")
  ...
  .delete('/clear-chat', zValidator('json', z.object({
    ...
    const db = c.env.DB;
    if (data.type === 'global') {
        await db.prepare("UPDATE user_settings SET global_chat_history = NULL WHERE user_id = ?")
            .bind(userId).run();
    } else if (data.type === 'card' && data.id) {
        await db.prepare("UPDATE user_media SET ai_chat_history = NULL WHERE id = ? AND user_id = ?")
  ...
  .get('/history', async (c) => {
      ...
      const db = c.env.DB;
      if (type === 'global') {
          const row = await db.prepare("SELECT global_chat_history FROM user_settings WHERE user_id = ?")
              .bind(userId).first<{global_chat_history: string | null}>();
          return c.json({ history: row?.global_chat_history ? JSON.parse(row.global_chat_history) : [] });
      } else if (type === 'card' && id) {
          const row = await db.prepare("SELECT ai_chat_history FROM user_media WHERE id = ? AND user_id = ?")
              .bind(id, userId).first<{ai_chat_history: string | null}>();
          return c.json({ history: row?.ai_chat_history ? JSON.parse(row.ai_chat_history) : [] });
      }
